...
int Code;
string Msg;
while (Form1->MC->ReportCommands->GetCommand(Command))
	{
	while (Form1->MC->ReportCommands->GetResult(Command))
	 {
	if (Command->GetResult(&Code,&Msg))
		{
		if (Command->GetResult(&Code,&Msg))
		if (Code!=COMMAND::SUCCESS) {
			//Logging.AddLog(Msg,Command->GetCommandString(),0);
			}
		AnsiString Str = (AnsiString)((AnsiString)(Msg.c_str()) + " " + Command->GetCommandString().c_str());
...
while (Form1->MC->UICommands->GetCommand(Command))
	{
...
//	if (Images[i]->Position->X <400) Images[i]->Position->X += 400;
	...
	//Images[i]->Opacity = Images[i]->Opacity.GetCurrent * Quiz->Items[i].Opacity;
