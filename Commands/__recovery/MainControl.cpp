...
TCommand::StringVars = new TStringVars;
...
ScriptControl->InitializeScripts(MAX_SCRIPT,this);
...
TMusic *Music;
Music = new TMusic();
MusicCommands = new TCommandQueue;
MusicControl = new TMusicControl(false,MusicCommands, Music);
}
...
{
QueryControl->Terminate();
QuizControl->Terminate();
Sleep(200);
//Terminate();
Sleep(200);
}
...
bool TMainControl::HandleCommand(TCommand* Command)
{
  ...
  if ((Command->GetCommandClass()=="") && (Command->GetCommandName()=="Set"))
	...
	*/
	...
	if (Command->GetCommandClass()=="Music")
		{
			if (MusicCommands!=NULL) MusicCommands->ForwardCommand(Command);
			return true;
		}
